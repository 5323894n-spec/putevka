...
На Render/Linux шаблон заполняется через `xlrd/xlwt/xlutils`. На Windows можно дополнительно использовать Microsoft Excel COM:

```bash
pip install -r backend/requirements-windows.txt
```

Остальные Excel-экспорты приложения остаются кроссплатформенными.
