...
## Точный путевой лист по Excel-образцу

Исходный шаблон `путевой лист.xls` находится в `templates/`.

В журнале путевых листов доступна кнопка **Шаблон XLS**. Она формирует файл на основе исходного `.xls` без упрощенной формы: сохраняются разметка, объединенные ячейки и оформление образца.

Для точного заполнения старого `.xls` на Windows нужен Microsoft Excel и `pywin32`:

```bash
pip install -r backend/requirements-windows.txt
```

На Linux-хостингах вроде Render этот точный COM-экспорт работать не будет без Windows/Excel. Остальные Excel-экспорты приложения остаются кроссплатформенными.

## Размещение в интернете

Инструкция для публикации backend на Render и frontend на Vercel находится в файле:
